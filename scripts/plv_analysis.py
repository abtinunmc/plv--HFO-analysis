...
Usage: python "plv analysis_last_step.py" [subject]
  e.g. python "plv analysis_last_step.py" sub-umich0019
"""
...
STEP3_DIR = os.path.join(SUBJECT_DIR, "step1_hfo_validation")
...
# Input directories (per-subject)
BANDPASS_DIR = os.path.join(SUBJECT_DIR, "step3_bandpass")

# Output directories (per-subject)
PLV_OUTPUT_DIR = os.path.join(SUBJECT_DIR, "step4_plv")
...
def extract_hfo_segments(run, run_data, bandpass_dir, subject, session, bp_dat_suffix, bp_lay_suffix):
    ...
    NOTE: The bandpass file contains CONCATENATED HFO segments (not the full recording).
    We read sequentially using durations from step3 .mat, not absolute timestamps.
    ...
    # Get HFO durations and good channel indices
    ...
    # Load segment lengths saved by car_reref.py (in exact write order)
    # This ensures we read segments in the same order they were written,
    # regardless of how CAR sorted them (by start time) vs step3 .mat (by index).
    car_output_dir = os.path.join(os.path.dirname(bandpass_dir), 'step2_car')
    seg_lengths_path = os.path.join(car_output_dir,
        f"{subject}_{session}_task-all_{run}_seg_lengths.npy")

    if os.path.isfile(seg_lengths_path):
        seg_lengths = np.load(seg_lengths_path).astype(int)
    else:
        # Fallback: compute from step3 timestamps (may have ordering mismatch)
        seg_lengths = np.array([max(0, int(round((t1 - t0) * fs)))
                                 for t0, t1 in zip(valid_start, valid_stop)], dtype=int)

    # Read segments SEQUENTIALLY (bandpass file has concatenated segments)
    with open(bp_dat, 'rb') as f:
        file_position = 0  # track current position in bytes

        for n_samp in seg_lengths:
            if n_samp <= 0:
                continue

            # Seek to current position and read segment
            ...
            # Update file position for next segment
            file_position += n_samp * n_ch_total * bytes_per_sample
...
# Hilbert edge trimming: remove unreliable samples at segment edges
# At 4096 Hz: 15 samples = ~3.7 ms trimmed from each edge
HILBERT_EDGE_TRIM = 15
MIN_SAMPLES_AFTER_TRIM = 20  # skip segments too short after trimming

print(f"  Edge trimming: {HILBERT_EDGE_TRIM} samples from each edge (~{1000*HILBERT_EDGE_TRIM/FS:.1f} ms)")
...
for run in RUNS:
    ...
    for seg in segments:
        ...
        # Trim unreliable edge samples
        if phase.shape[0] > 2 * HILBERT_EDGE_TRIM + MIN_SAMPLES_AFTER_TRIM:
            phase = phase[HILBERT_EDGE_TRIM:-HILBERT_EDGE_TRIM, :]
        elif phase.shape[0] < MIN_SAMPLES_AFTER_TRIM:
            # Segment too short, skip it
            n_skipped_short += 1
            continue
        # else: segment is short but usable, don't trim
    ...
    if len(phases) > 0:
        ...
        print(f"  {run}: {len(phases)} segments processed")
        print(f"    Phase range: [{first_phase.min():.2f}, {first_phase.max():.2f}] rad")
        print(f"    Phase shape: {first_phase.shape} (samples, channels)")
    else:
        print(f"  {run}: no valid segments after trimming")

if n_skipped_short > 0:
    print(f"  Note: {n_skipped_short} segments skipped (too short after edge trim)")
